...
This document contains the code used in SAS Programming for Enterprise Guide Users.
...
PROC SQL;
	...
	AS SELECT Name, Sex, Age, Height, Weight FROM EGPROG.CLASS;
QUIT;
TITLE;
...
PROC SQL;
CREATE TABLE SASUSER.QUERY_FOR_CLASS_0005 AS SELECT CLASS.Name,
	CLASS.Sex,
	CLASS.Age,
	CLASS.Height,
	CLASS.Weight 
  FROM EGPROG.CLASS AS CLASS
  WHERE CLASS.Sex = "F";
QUIT;
...
CREATE TABLE SASUSER.QUERY_FOR_CLASS_0005 AS SELECT CLASS.Name,
	...
	CLASS.Weight 
 	FROM EGPROG.CLASS AS CLASS
WHERE ( CLASS.Sex = "F" AND CLASS.Age BETWEEN 12 AND 15 AND CLASS.Height > 56.5 ) OR ( CLASS.Sex = "M" AND CLASS.Age BETWEEN 12 AND 15 AND CLASS.Height > 59 );
QUIT;
...
WHERE (Sex = "F" AND Age BETWEEN 12 AND 15 AND Height > 56.5 ) OR ( Sex = "M" AND Age BETWEEN 12 AND 15 AND Height > 59 );
...
ODS HTML FILE="c:\temp\%SYSFUNC(TODAY(),YYMMDDN8.).html";
...
ODS DOCUMENT NAME=myoutput;

ODS DOCUMENT CLOSE;

PROC DOCUMENT NAME=myoutput;
		REPLAY;
RUN;
QUIT;

ODS PDF FILE="c:/temp/mypdf.pdf" BOOKMARKGEN=NO;
ODS DOCUMENT NAME=interim;

ODS DOCUMENT CLOSE;
ODS PDF CLOSE;
ODS HTML PATH="c:/temp" FILE='myhtml.html';
PROC DOCUMENT NAME=interim;
   DIR \print;
   OBFOOTN print "<a href='mypdf.pdf' target='_blank'>Printable Version</a>"; 
   REPLAY;
RUN;
QUIT;
ODS HTML CLOSE;

ODS HTML PATH="c:/temp" BODY='myhtml.html';
PROC DOCUMENT NAME=interim;
		LIST / LEVELS=ALL;
		RUN;
QUIT;
...
PROC SQL NUMBER;
	...
	CASE row
 		WHEN 1 THEN "href='c:\temp\prod.html'"
...
%MACRO PRINTOUT(doc,file);
ODS DOCUMENT CLOSE;
ODS PDF CLOSE;
ODS HTML PATH="c:/temp" FILE=”&file..html”;
PROC DOCUMENT NAME=&doc;
   DIR \print;
   OBFOOTN print "<a href='&file..pdf' target='_blank'>Printable Version</a>"; 
   REPLAY;
RUN;
QUIT;
ODS HTML CLOSE;  
...
%INCLUDE 'c:\temp\printout.sas';
%printout(interim, myfile)
