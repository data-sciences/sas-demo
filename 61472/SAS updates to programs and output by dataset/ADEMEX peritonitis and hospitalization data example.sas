...
/*--- Code for Output 4.2  ---*/    
/*----------------------------*/   
options nocenter linesize=40 nodate nonumber;
...
run;
/*=========================================================
  The following SORT command has been corrected from the 
  code shown in the book (first printing) and made available
  initially on the author web page. The results produced 
  with this correct sort will differ from the results    
  initially published in the book for Output 7.2 as it now
  correctly identifies two observations per subject due to
  the correct sorting. This correction to the SORT command 
  does not impact the results shown in Output 7.1 of the 
  book since the model corresponding to Output 7.1 does not
  involve a random effect for the hospitalization data.
  The corrected output is located in the file:
     "Update to Output 7.2.LST" 
  under the folder name:
     "SAS updates to programs and output by dataset"
==========================================================*/
proc sort data=example7_1_1_SP;
*by ind ptid; ** This is WRONG (as shown in first printing);
 by ptid ind; ** This is CORRECT;
run;
...
run;
data phi; set phi;
*phi = phi_num/(n-7); ** Wrong DF based on 7 rather than 8 parameters;
 phi = phi_num/(n-8); ** Right DF based on 8 parameters;
 Scale = sqrt(phi);
run;
...
proc nlmixed data=example7_1_1_SP method=Gauss 
 ...
 phi_num = (((response-mu2_i)**2)/mu2_i)*(ind=0);
 /*========================================================
  Define the shared parameter means for y1i and y2i    
  where y1i is the number of episodes of peritonitis    
  (response=Episodes) and y2i is the number of hospital 
  admissions (response=Hosp) from the ADEMEX dataset.   
 =========================================================*/
...
/*       Output 7.1 (first call to NLMIXED above), but it is    */
/*       more efficient in that it does not require using an    */
/*       alternative dataset. It uses the original dataset.     */
...
/*================================================================*/
/* Note 1: This alternative code gives same estimates as in       */
/*         Output 7.2 (second call to NLMIXED above), but it is   */
/*         more efficient in that it does not require using an    */
/*         alternative dataset. It uses the original dataset and  */
/*         provides the same estimates provided one used the      */
/*         correct sorting procedure where we sort: BY PTID IND;  */
/*         This alternative code is not referenced within the     */
/*         book but is included here for future reference as it   */ 
/*         provides a far more efficient estimation procedure.    */ 
/* Note 2: This alternative code also includes a means for        */ 
/*         correcting for conditional Poisson overdispersion in   */ 
/*         the standard errors used for inference for the         */ 
/*         hospitalization portion of this SP model.              */ 
/*================================================================*/
ods output parameterestimates=pe_SP;
proc nlmixed data=example7_1_1 method=Gauss 
             tech=newrap noad qpoints=20;
 parms /data=peJOINT1;
 ui = CDF('NORMAL',zi);
 ...
 x_beta = beta_Intercept + beta_Trt*Trt + beta_Sex*Sex + 
          ...
          beta_Albumin*Albumin + beta_PriorMonths*PriorMonths +
          log_time; 
 /* Include bi as a 'covariate' for hospitalization */ 
 mu2_i = exp(x_beta + beta_bi*bi);
 /*========================================================
  Define the shared parameter model for y1i and y2i    
  where y1i is the number of episodes of peritonitis    
  (response=Episodes) and y2i is the number of hospital 
  admissions (response=Hosp) from the ADEMEX dataset.   
 =========================================================*/
 ...
 /*========================================================
  Define the numerator of the Poisson overdispersion      
  parameter phi as defined in equation (4.6) of Ch. 4     
  for the hospitalization data (based on Pearson Chi-square)                           
 =========================================================*/
 phi_num = (((hosp-mu2_i)**2)/mu2_i);
 model hosp ~ general(ll);
 random zi ~ N(0,1) subject=ptid;
 id mu1_i mu2_i ll_1 ll_2 ll phi_num;
 predict mu2_i out=pred1;
 title 'SP model for peritonitis and hospitalization - alternative program specifications ';
run;
...
/*=========================================================
 ...
 and associated scale (square root) parameter as a means 
 for assessing model goodness-of-fit with respect to 
 the hospitalization data under the SP model. Also print
 out parameter estimates and revised standard errors that
 reflect conditional Poisson overdispersion for the 
 hospitalization data along with revise t-tests and p-values.
 ==========================================================*/
proc means data=pred1 sum noprint;
 var phi_num; 
 output out=phi1 n=n sum=phi_num;
...
data peSP;
 set pe_SP;
 if _n_=1 then set phi1(keep=phi Scale);
run;
data peSP1;
 set peSP;
 if scan(Parameter,1,'_')='beta' then do;
...
run;
proc print data=peSP noobs;
 var Parameter Estimate StandardError tValue Probt;
run;
proc print data=peSP1 noobs;
 var Parameter Estimate Scale StandardError tValue Probt;
 format Scale 5.3;
run;
