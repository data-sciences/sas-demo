...
proc sort data=advrpt.demog(keep=lname fname sex symp race)
   ...
   run;
data namelistc(keep=lname fname varname varvalue);
   set codedat;
   ...
   do i = 1 to cnt;
      ...
      varname = substr(vformatx(vlist{i}),2,(anydigit(vformatx(vlist{i}))-2));
      output namelistc;
   end;
   run;
proc transpose data=namelistc out=originalc(drop=_name_);
by lname fname;
id varname;
var varvalue;
run;

proc print data=originalc;
run;
